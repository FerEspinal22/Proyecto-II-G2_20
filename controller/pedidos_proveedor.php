...
if ($_SERVER['REQUEST_METHOD'] === 'OPTIONS') {
    ...
    switch($_GET["op"]){
        ...
        case "InsertPedido":
            ...
            echo json_encode("Pedido del proveedor agregado correctamente");
        break;

        case "UpdatePedido":
            $datos=$pedidos->update_pedido($body["ID"], $body["ID_SOCIO"], $body["FECHA_PEDIDO"], $body["DETALLE"],
            $body["SUB_TOTAL"], $body["TOTAL_ISV"], $body["TOTAL"], $body["FECHA_ENTREGA"], $body["ESTADO"]);
            echo json_encode("Pedido del proveedor actualizado correctamente");
        break;

        case "DeletePedido":
            $datos=$pedidos->delete_pedido($body["ID"]);
            echo json_encode("Pedido del proveedor eliminado correctamente");
        break;
    }
?>